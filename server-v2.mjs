...
const INITIAL_ACTIVITY_PAGES = 3;
const ACTIVITIES_PER_PAGE = 200;
const MAX_CACHED_ACTIVITIES = INITIAL_ACTIVITY_PAGES * ACTIVITIES_PER_PAGE;
...
app.get('/', async (req, res, next) => {
  try {
    ...
    const tag = '<script src="/volume-weeks.js?v=1"></script>';
...
function mergeActivities(recent, historical = []) {
  const seen = new Set();
  const merged = [];
  for (const a of [...recent, ...historical]) {
    const key = String(a.id ?? `${a.start_date_local || a.start_date}-${a.name || ''}`);
    if (seen.has(key)) continue;
    seen.add(key);
    merged.push(a);
  }
  merged.sort((a, b) => new Date(b.start_date_local || b.start_date) - new Date(a.start_date_local || a.start_date));
  return merged.slice(0, MAX_CACHED_ACTIVITIES);
}

async function loadInitialActivityHistory(session) {
  const all = [];
  for (let page = 1; page <= INITIAL_ACTIVITY_PAGES; page++) {
    const chunk = await stravaGet(`/athlete/activities?per_page=${ACTIVITIES_PER_PAGE}&page=${page}`, session);
    all.push(...chunk);
    if (chunk.length < ACTIVITIES_PER_PAGE) break;
  }
  return all;
}
...
async function getActivities(session, force = false) {
  ...
  let data;
  if (!cached) {
    data = await loadInitialActivityHistory(session);
  } else {
    const recent = await stravaGet(`/athlete/activities?per_page=${ACTIVITIES_PER_PAGE}&page=1`, session);
    data = mergeActivities(recent, cached.data);
  }
...
app.get('/api/activities', async (req, res) => {
  try {
    ...
    res.json({
      ...
      syncedAt: new Date(result.at).toISOString(),
      cached: req.query.force !== '1',
      historyLimit: MAX_CACHED_ACTIVITIES
    });
  } catch (e) {
...
app.get('/health', (req, res) => res.json({
  ...
  activityHistoryLimit: MAX_CACHED_ACTIVITIES
}));
